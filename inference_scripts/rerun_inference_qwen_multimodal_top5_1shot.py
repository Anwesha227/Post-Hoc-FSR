...
OUTPUT_CSV = "../qwen_output/qwen_multimodal_top5_1shot_results_rerun.csv"
SKIPPED_LOG = "qwen_error_log.txt"
TIMEOUT_SEC = 60
...
# === Load skipped filenames robustly ===
...
# Ensure pred{i}_sci exists by falling back to pred{i}_name
for i in range(1, 6):
    df[f"pred{i}_sci"] = df.get(f"pred{i}_sci", df[f"pred{i}_name"])

# Filter for only the skipped rows
df = df[df["filename"].apply(lambda x: os.path.basename(x) in skipped_filenames)]
...
def get_one_image_from_class(class_id):
    ...
    if len(all_images) == 0:
        raise ValueError(f"No images found in class {class_id}")
    return encode_image(os.path.join(class_path, random.choice(all_images)))
...
def build_multimodal_prompt(query_image, candidates):
    messages = [
        {"type": "text", "text":
         ...
         "Step 2: Compare it to each of the 5 candidate species. Each candidate includes the species name and one example image.\n"
...
print(f"\n🔁 Rerunning {len(df)} skipped images...")
...
for idx, row in tqdm(df.iterrows(), total=len(df)):
    try:
        ...
        # Get references for 5 candidates
        candidates = []
        for i in range(1, 6):
...
# === Save skipped items again if any ===
log_dir = os.path.dirname(SKIPPED_LOG)
if log_dir:
...
print(f"\n✅ Rerun done. Results saved to {OUTPUT_CSV}. Skipped {len(skipped)} cases.")
