...
OUTPUT_CSV = "../qwen_output/qwen_multimodal_1shot_results.csv"
SKIPPED_LOG = "../error_logs/qwen_multimodal_1shot_skipped.txt"
TIMEOUT_SEC = 60
...
def get_one_image_from_class(class_id):
    ...
    if len(all_images) < 1:
        raise ValueError(f"No images in class {class_id}")
    selected = random.choice(all_images)
    return [encode_image(os.path.join(class_path, selected))]

def build_multimodal_prompt(query_image, candidates):
    """
    candidates: list of tuples [(sci_name, [img1_base64]), ...]
    """
    messages = [
        {"type": "text", "text":
         ...
         "Step 2: Compare it to each of the 3 candidate species. Each candidate includes the species name and one example image.\n"
    ...
    for idx, (name, imgs) in enumerate(candidates, 1):
...
for idx, row in tqdm(df.iterrows(), total=len(df)):
    try:
        ...
        # Get class IDs for each candidate
        try:
            ...
            skipped.append((row["image_path"], f"Missing species: {str(e)}"))
            continue

        cand1_refs = get_one_image_from_class(cand1_class)
        cand2_refs = get_one_image_from_class(cand2_class)
        cand3_refs = get_one_image_from_class(cand3_class)

        candidates = [
            (row["pred1_sci"], cand1_refs),
            (row["pred2_sci"], cand2_refs),
            (row["pred3_sci"], cand3_refs),
        ]
