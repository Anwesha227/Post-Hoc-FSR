...
OUTPUT_CSV = "../qwen_output/qwen_multimodal_top5_3shot_results.csv"
SKIPPED_LOG = "../error_logs/qwen_multimodal_top5_3shot_skipped.txt"
TIMEOUT_SEC = 60
...
def get_stitched_image_from_class(class_id):
    ...
    if len(all_images) < 2:
        raise ValueError(f"Not enough images to stitch in class {class_id}")

    selected = random.sample(all_images, 3)
    # print(f"🖼️ Selected for class {class_id}: {selected}")
    ...
    stitched = Image.new("RGB", (total_width, min_height))
    ...
    # Encode stitched image to base64
    buffer = BytesIO()
    stitched.save(buffer, format="JPEG")
...
def build_multimodal_prompt(query_image, candidates):
    messages = [
        {"type": "text", "text":
         ...
         "Step 2: Compare it to each of the 5 candidate species. Each candidate includes the species name and a stitched image showing three different individuals of that species.\n"
