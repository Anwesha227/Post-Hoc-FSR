...
FEWSHOT_FILE = "fewshot16_seed1.txt"  
OUTPUT_DIR = "pregenerated_references_12shot"
NUM_CLASSES = 200

os.makedirs(OUTPUT_DIR, exist_ok=True)

# === Step 1: Parse fewshot file ===
...
def create_stitched_reference(class_id):
    ...
    # Load 8 additional images from fewshot file (prefer no overlap)
    ...
    selected_extra = random.sample(extra_candidates, 8)
    extra_imgs = [Image.open(p).convert("RGB") for p in selected_extra]

    imgs = human_imgs + extra_imgs
    assert len(imgs) == 12

    # Split into 4 rows of 3 images
    rows = [imgs[i * 3:(i + 1) * 3] for i in range(4)]
...
print(f"\nDone generating 12-shot (4×3) tiles.")
if skipped:
